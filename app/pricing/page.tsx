  ...
  return (
    <div className="w-full py-[3rem] h-100 px-8 bg-black  bg-[url('/assets/Pricing-bg.png')] bg-cover bg-no-repeat bg-center bg-black ">
      <h2 className="text-5xl font-xs text-white text-center py-2 mx-auto">
        We offer great <span className="text-blue-400">price</span> plan <br />
        for the application
      </h2>
      <div
        className="mt-4 mb-6 ml-5 "
        style={{
          display: "flex",
      ...
      <div className="max-w-[1240px] mx-auto grid md:grid-cols-3 gap-8 bg-gray-200 px-8  ">
        {cardData1.map((card, index) => (
          ...
          >
            <h2 className="text-2xl font-xs text-center py-4">{card.title}</h2>
            <p className="text-center text-sm font-xs mb-5">{card.paragraph}</p>

            <p className="text-center text-4xl font-bold mt-6 mb-6">
              {card.price1}
            </p>
        ...
        {cardData2.map((card, index) => (
          ...
          >
            ...
            <h2 className="text-2xl font-xs text-center py-2">{card.title}</h2>
            <p className="text-center text-sm font-xs mb-2">
              {card.paragraph1}
            </p>
            <p className="text-center text-sm font-xs mb-2">
              {card.paragraph2}
        ...
        {cardData3.map((card, index) => (
          ...
          >
            <h2 className="text-2xl font-xs text-center py-4">{card.title}</h2>
            <p className="text-center text-sm font-xs mb-1">
              {card.paragraph1}
            </p>
            <p className="text-center text-sm font-xs mb-1">
              {card.paragraph2}
